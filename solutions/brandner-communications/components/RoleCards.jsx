  ...
  return (
    <section id="services" className="bg-surface py-28 px-6" aria-labelledby="services-heading">
      <div className="max-w-7xl mx-auto">
        <div ref={headRef} className="opacity-0 translate-y-8 transition-all duration-700 mb-16">
          <div className="flex items-center gap-4 mb-5">
            <div className="w-6 h-px bg-accent" aria-hidden="true" />
            <span className="text-accent text-[10px] font-medium tracking-[0.35em] uppercase">What We Do</span>
          </div>
          <h2 id="services-heading" className="font-display font-bold text-4xl sm:text-6xl text-ink leading-[1.0] max-w-2xl">
        ...
        <div
          ref={gridRef}
          className="opacity-0 translate-y-8 transition-all duration-700 delay-100 grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border"
        >
          {services.map(({ num, title, description }) => (
            <div
              key={num}
              className="group bg-surface hover:bg-surface-2 p-8 sm:p-10 transition-colors duration-300 cursor-default"
            >
              <div className="text-border text-[10px] font-medium tracking-[0.3em] mb-6 group-hover:text-accent transition-colors duration-300">
                {num}
              </div>
              <h3 className="font-display font-bold text-2xl text-ink mb-3 group-hover:text-ink transition-colors">
                {title}
              </h3>
